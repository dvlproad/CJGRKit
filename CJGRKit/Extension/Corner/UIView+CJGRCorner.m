...
@class CJGRCornerTransformObserver;
...
@property (nonatomic, strong) CJGRCornerPanResizeCalculator *cjCornerPanResizeCalculator;
@property (nonatomic, strong) CJGRCornerTransformObserver *cjCornerTransformObserver;
...
@interface CJGRCornerTransformObserver : NSObject
...
@implementation CJGRCornerTransformObserver
...
@implementation UIView (CJGRCorner)
...
- (CJGRCornerPanResizeCalculator *)cjCornerPanResizeCalculator {
    CJGRCornerPanResizeCalculator *calculator = objc_getAssociatedObject(self, &cjCornerPanResizeCalculatorKey);
    if (!calculator) {
        calculator = [[CJGRCornerPanResizeCalculator alloc] init];
        objc_setAssociatedObject(self, &cjCornerPanResizeCalculatorKey, calculator, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    return calculator;
}
- (void)setCjCornerPanResizeCalculator:(CJGRCornerPanResizeCalculator *)cjCornerPanResizeCalculator {
...
- (CJGRCornerTransformObserver *)cjCornerTransformObserver {
    return objc_getAssociatedObject(self, &cjCornerTransformObserverKey);
}

- (void)setCjCornerTransformObserver:(CJGRCornerTransformObserver *)cjCornerTransformObserver {
...
- (void)__cornerMinimizeAction:(UIPanGestureRecognizer *)panGR {
    switch (panGR.state) {
        ...
        case UIGestureRecognizerStateChanged:
        {
            ...
            CJGRCornerPanResizeResult result = [self.cjCornerPanResizeCalculator panChangedWithTranslation:translation];
...
- (void)__cj_startCornerTransformObservationIfNeeded {
    ...
    CJGRCornerTransformObserver *observer = [[CJGRCornerTransformObserver alloc] init];
...
static IMP CJGRCornerOriginalHitTestIMP = NULL;

static UIView *CJGRCornerHitTest(id self, SEL _cmd, CGPoint point, UIEvent *event) {
    UIView *result = ((UIView *(*)(id, SEL, CGPoint, UIEvent *))CJGRCornerOriginalHitTestIMP)(self, _cmd, point, event);
    if (result) {
        return result;
    }
...
- (void)__cj_swizzleHitTestIfNeeded {
    ...
    dispatch_once(&onceToken, ^{
        ...
        CJGRCornerOriginalHitTestIMP = method_getImplementation(method);
        method_setImplementation(method, (IMP)CJGRCornerHitTest);
    });
}
