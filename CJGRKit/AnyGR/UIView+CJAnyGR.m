...
@property (nonatomic, copy) void(^cjGRStateChangeBlock)(CJGRType type, UIGestureRecognizerState state);
@property (nonatomic, copy) void(^cjGRStateChangeBlock2)(CJGRType type, UIGestureRecognizerState state);

@end

@implementation UIView (CJAnyGR)
...
static NSString * const cjGRStateChangeBlock2Key = @"cjGRStateChangeBlock2Key";
...
- (void (^)(CJGRType, UIGestureRecognizerState))cjGRStateChangeBlock {
    return objc_getAssociatedObject(self, &cjGRStateChangeBlockKey);
}

- (void)setCjGRStateChangeBlock:(void (^)(CJGRType, UIGestureRecognizerState))cjGRStateChangeBlock {
    objc_setAssociatedObject(self, &cjGRStateChangeBlockKey, cjGRStateChangeBlock, OBJC_ASSOCIATION_COPY_NONATOMIC);
}

//cjGRStateChangeBlock2
- (void (^)(CJGRType, UIGestureRecognizerState))cjGRStateChangeBlock2 {
    return objc_getAssociatedObject(self, &cjGRStateChangeBlock2Key);
}

- (void)setCjGRStateChangeBlock2:(void (^)(CJGRType, UIGestureRecognizerState))cjGRStateChangeBlock2 {
    objc_setAssociatedObject(self, &cjGRStateChangeBlock2Key, cjGRStateChangeBlock2, OBJC_ASSOCIATION_COPY_NONATOMIC);
}

/// 手势状态变化时，统一回调给外部（用于感知变换开始/进行/结束，type 标识手势来源）
- (void)__cj_notifyGRStateChange:(CJGRType)type state:(UIGestureRecognizerState)state {
    if (self.cjGRStateChangeBlock) {
        self.cjGRStateChangeBlock(type, state);
    }
    if (self.cjGRStateChangeBlock2) {
        self.cjGRStateChangeBlock2(type, state);
    }
}
...
- (void)cj_panGRAction:(UIPanGestureRecognizer *)panGR {
    ...
    [self __cj_notifyGRStateChange:CJGRTypePan state:panGR.state];
}
...
- (void)cj_pinchGRAction:(UIPinchGestureRecognizer *)pinchGR {
    ...
    [self __cj_notifyGRStateChange:CJGRTypePinch state:pinchGR.state];
}
...
- (void)cj_rotationGRAction:(UIRotationGestureRecognizer *)rotationGR {
    ...
    [self __cj_notifyGRStateChange:CJGRTypeRotation state:rotationGR.state];
}
